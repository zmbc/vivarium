...
class DiseaseTransition(Transition):
    ...
    def setup(self, builder):
        ...
        self.transition_rate = builder.value.register_rate_producer(self.rate_name, source=self._risk_deleted_rate)
...
class DiseaseState(State):
    ...
    def setup(self, builder):
        ...
        self.excess_mortality_rate = builder.value.register_rate_producer(
            f"{self.state_id}.excess_mortality_rate", source=self.risk_deleted_excess_mortality_rate
        )
...
class DiseaseModel(Machine):
    def __init__(self, disease, initial_state, cause_specific_mortality_rate=0.0, **kwargs):
...
class SIS_DiseaseModel:
    ...
    def __init__(self, disease_name):
        ...
        susceptible_state.add_transition(
            infected_state, measure="incidence_rate", rate_name=f"{infected_state.state_id}.incidence_rate"
        )
        infected_state.allow_self_transitions()
        infected_state.add_transition(
            susceptible_state, measure="remission_rate", rate_name=f"{infected_state.state_id}.remission_rate"
        )

        model = DiseaseModel(self._name, initial_state=susceptible_state, states=[susceptible_state, infected_state])
